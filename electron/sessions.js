...
function resolveDefaultSessionShell(env) {
  const sourceEnv = env && typeof env === 'object' ? env : process.env;
  if (process.platform === 'win32') {
    return sourceEnv.COMSPEC || sourceEnv.ComSpec || 'cmd.exe';
  }
  const shell = typeof sourceEnv.SHELL === 'string' && sourceEnv.SHELL.trim()
    ? sourceEnv.SHELL.trim()
    : '';
...
async function startSession({ sessionRoot, name, command, cwd, windowName, env } = {}) {
