...
export const win32SystemTerminalLauncher = {
  ...
  async launchCliInSystemTerminal({ runId, cwd, cliPath, sessionRoot, terminalsDir, landConfigId, env } = {}) {
    ...
    const script = [
      ...
      ...(configHostApp ? [`set \"MODEL_CLI_CONFIG_HOST_APP=${escapeCmdBatchString(configHostApp)}\"`] : []),
    ...
    const comspec =
      typeof runtimeEnv.COMSPEC === 'string' && runtimeEnv.COMSPEC.trim() ? runtimeEnv.COMSPEC.trim() : 'cmd.exe';
