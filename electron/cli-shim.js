...
export function createCliShim({ projectRoot, commandName, env } = {}) {
  ...
  function resolveWindowsAppsDir() {
    const localAppData =
      typeof runtimeEnv.LOCALAPPDATA === 'string' && runtimeEnv.LOCALAPPDATA.trim()
        ? runtimeEnv.LOCALAPPDATA.trim()
        : '';
  ...
  function resolveWindowsFallbackBinDir() {
    const root = resolveStateRootDir({ env: runtimeEnv, homeDir: os.homedir() });
