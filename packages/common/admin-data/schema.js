...
export const TABLE_SCHEMAS = {
  ...
  settings: {
    ...
    columns: [
      ...
      { name: 'subagentDefaultModel', type: 'string', required: false, note: '子流程默认模型' },
      ...
      { name: 'shellSafetyMode', type: 'enum(strict|relaxed)', required: false, note: 'Shell 安全模式' },
      { name: 'shellMaxBufferBytes', type: 'number', required: false, note: 'Shell 输出缓冲上限 (bytes)' },
      { name: 'filesystemSymlinkPolicy', type: 'enum(allow|deny)', required: false, note: '文件系统 symlink 逃逸策略' },
      { name: 'filesystemMaxFileBytes', type: 'number', required: false, note: '文件读取上限 (bytes)' },
      { name: 'filesystemMaxWriteBytes', type: 'number', required: false, note: '文件写入上限 (bytes)' },
      { name: 'mcpToolLogLevel', type: 'enum(off|info|debug)', required: false, note: 'MCP 工具日志级别' },
      { name: 'mcpToolLogMaxBytes', type: 'number', required: false, note: 'MCP 工具日志最大字节' },
      { name: 'mcpToolLogMaxLines', type: 'number', required: false, note: 'MCP 工具日志最大行数' },
      { name: 'mcpToolLogMaxFieldChars', type: 'number', required: false, note: 'MCP 工具日志单字段截断上限' },
      { name: 'mcpStartupConcurrency', type: 'number', required: false, note: 'MCP 启动并发' },
      { name: 'uiPromptLogMode', type: 'enum(full|minimal)', required: false, note: 'UI prompt 日志模式' },
...
export const runtimeSettingsSchema = z.object({
  ...
  subagentDefaultModel: z.string().trim().optional().default(''),
  ...
  shellSafetyMode: z.enum(['strict', 'relaxed']).optional().default('strict'),
  shellMaxBufferBytes: z.number().int().min(16 * 1024).max(50 * 1024 * 1024).optional().default(2 * 1024 * 1024),
  filesystemSymlinkPolicy: z.enum(['allow', 'deny']).optional().default('allow'),
  filesystemMaxFileBytes: z.number().int().min(1024).max(50 * 1024 * 1024).optional().default(256 * 1024),
  filesystemMaxWriteBytes: z.number().int().min(1024).max(100 * 1024 * 1024).optional().default(5 * 1024 * 1024),
  mcpToolLogLevel: z.enum(['off', 'info', 'debug']).optional().default('info'),
  mcpToolLogMaxBytes: z.number().int().min(0).max(200 * 1024 * 1024).optional().default(5 * 1024 * 1024),
  mcpToolLogMaxLines: z.number().int().min(0).max(200000).optional().default(5000),
  mcpToolLogMaxFieldChars: z.number().int().min(200).max(200000).optional().default(4000),
  mcpStartupConcurrency: z.number().int().min(1).max(20).optional().default(4),
  uiPromptLogMode: z.enum(['full', 'minimal']).optional().default('full'),
...
export const DEFAULT_RUNTIME_SETTINGS = {
  ...
  shellSafetyMode: 'strict',
  shellMaxBufferBytes: 2 * 1024 * 1024,
  filesystemSymlinkPolicy: 'allow',
  filesystemMaxFileBytes: 256 * 1024,
  filesystemMaxWriteBytes: 5 * 1024 * 1024,
  mcpToolLogLevel: 'info',
  mcpToolLogMaxBytes: 5 * 1024 * 1024,
  mcpToolLogMaxLines: 5000,
  mcpToolLogMaxFieldChars: 4000,
  mcpStartupConcurrency: 4,
  uiPromptLogMode: 'full',
